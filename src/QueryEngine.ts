 ...
 *
 * 【设计目的】
 * QueryEngine 拥有查询生命周期和对话会话状态。
 * 它从 ask() 中提取核心逻辑到一个独立的类中，
 * 可以被 headless/SDK 路径使用（在未来阶段也可以被 REPL 使用）。
 *
 * 【核心概念】
 * - 一个 QueryEngine 对应一个对话
 * - 每次 submitMessage() 调用在同一个对话中开始一个新的 turn
 * - 状态（消息、文件缓存、使用量等）在 turns 之间持久化
 *
 * 【关键设计决策】
 * 1. 使用 AsyncGenerator 实现流式响应
 *    - 优点：可以逐步返回结果，无需等待完整响应
 *    - 缺点：需要处理部分完成的状态
 *
 * 2. 死码消除（Dead Code Elimination）
 *    - 条件导入用于协调器模式和历史压缩特性
 *    - 允许生产构建排除未使用的代码
 *
 * 3. 消息历史管理
 *    - mutableMessages: 可变消息队列，用于累积对话
 *    - 支持对话恢复和转录记录
 *
 * @example
